...
fun LoginPage(modifier: Modifier=Modifier,navController: NavController,authViewModel: MusicViewModel)
{
    ...
    LaunchedEffect(authen.value) {
        when(authen.value){
            ...
            is AuthState.Error -> Toast.makeText(context,
                (authen.value as AuthState.Error).message, Toast.LENGTH_SHORT).show()
            else->Unit
        }
    }
    ...
    Column(modifier=modifier.fillMaxSize(),
        ...
        Text(text = "Login", fontSize = 53.sp, color = Color(0xFF43D849))
        Column(modifier=modifier.fillMaxSize(),
            horizontalAlignment = Alignment.CenterHorizontally,
            ...
            Button(onClick = {
                authViewModel.loginbro(email,password)
            }) {
                Text(text = "Login")
            }
